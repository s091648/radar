...
for file in [r'\RCWF\20190809_0101_RCWF_VOL.239',r'\RCTP\TIA180710190009.RAW48V7']:
    ...
    for swp in range(radar.nsweeps):
        ...
        display.plot_ppi_map(dataname, swp, fig=fig, ax=111, projection=map_proj, resolution='10m', min_lon=lon-dx, max_lon=lon+dx, lon_lines=np.linspace(lon-dx,lon+dx,2*dx+1), lat_lines=np.linspace(lat-dx,lat+dx,2*dx+1),min_lat=lat-dx, max_lat=lat+dx, cmap=cmap, norm=norm, vmin=min(clv), vmax=min(clv), ticks=clv, colorbar_label=varname+' ('+unit+')')
