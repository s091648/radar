...
for file in [r'\RCWF\20190809_0101_RCWF_VOL.239',r'\RCTP\TIA180710190009.RAW48V7']:
    ...
    for swp in range(radar.nsweeps):
        ...
        display.plot_ppi_map(dataname, swp, fig=fig, ax=111, projection=map_proj, resolution='10m', min_lon=round(lon)-dx, max_lon=round(lon)+dx, lon_lines=np.linspace(round(lon)-dx,round(lon)+dx,2*dx+1), lat_lines=np.linspace(round(lat)-dx,round(lat)+dx,2*dx+1),min_lat=round(lat)-dx, max_lat=round(lat)+dx, cmap=cmap, norm=norm, vmin=min(clv), vmax=min(clv), ticks=clv, colorbar_label=varname+' ('+unit+')')

        #plt.show()
        plt.savefig(des+'\\'+name+'_'+savename+'_'+str(swp+1)+'.png')